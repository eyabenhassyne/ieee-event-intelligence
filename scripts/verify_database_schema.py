...
def fetch_value(cur, sql: str, params: tuple = ()) -> object | None:
    cur.execute(sql, params)
    row = cur.fetchone()
    return row[0] if row else None


def check(cur, label: str, condition: bool) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            def run(label: str, condition: bool) -> None:
                nonlocal failed
                if not check(cur, label, condition):
                    failed = True

            for schema in ["raw", "staging", "analytics", "audit"]:
                run(f"Schema exists: {schema}", fetch_value(cur, "SELECT EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = %s)", (schema,)))
            ...
            for schema, table in expected_tables:
                run(f"Table exists: {schema}.{table}", fetch_value(cur, "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = %s AND table_name = %s)", (schema, table)))
            ...
            run("DimSubcategory references DimCategory", bool(fetch_value(cur, """
                SELECT EXISTS (
                    SELECT 1
            ...
            """)))
            run("DimState references DimCountry", bool(fetch_value(cur, """
                SELECT EXISTS (
                    SELECT 1
                    FROM information_schema.table_constraints tc
                    WHERE tc.table_schema = 'analytics'
            ...
            """)))
            run("FactEvents has unique source_event_id", bool(fetch_value(cur, """
                SELECT EXISTS (
                    SELECT 1
                    FROM pg_indexes
            ...
            for table, column, predicate in unknown_tests:
                run(f"Unknown member exists: {table}", bool(fetch_value(cur, f"SELECT EXISTS (SELECT 1 FROM {table} WHERE {predicate} AND {column} = TRUE)")))
